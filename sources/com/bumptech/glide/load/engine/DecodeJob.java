...
class DecodeJob<R> implements DataFetcherGenerator.FetcherReadyCallback, Runnable, Comparable<DecodeJob<?>>, FactoryPools.Poolable {
    ...
    private void logWithTimeAndKey(String message, long startTime, String extraArgs) {
        Log.v(TAG, message + " in " + LogTime.getElapsedMillis(startTime) + ", load key: " + this.loadKey + (extraArgs != null ? ", " + extraArgs : HttpUrl.FRAGMENT_ENCODE_SET) + ", thread: " + Thread.currentThread().getName());
    }
    ...
    <Z> Resource<Z> onResourceDecoded(DataSource dataSource, Resource<Z> decoded) {
        ...
        if (this.decodeHelper.isResourceEncoderAvailable(transformed)) {
            ResourceEncoder encoder = this.decodeHelper.getResultEncoder(transformed);
        ...
        if (this.diskCacheStrategy.isResourceCacheable(isFromAlternateCacheKey, dataSource, encodeStrategy)) {
            if (resourceEncoder != null) {
                ...
                LockedResource lockedResourceObtain = LockedResource.obtain(transformed);
