...
public class ShizukuSystemProperties {
    ...
    public static int getInt(String key, int def) throws RemoteException {
        return Integer.decode(Shizuku.requireService().getSystemProperty(key, Integer.toString(def))).intValue();
    }

    public static long getLong(String key, long def) throws RemoteException {
        return Long.decode(Shizuku.requireService().getSystemProperty(key, Long.toString(def))).longValue();
    }
